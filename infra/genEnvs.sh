...
export EBS_PERSISTENT_VOLUME_ID="vol-0d3a782bdfffc34aa"
...
mkdir -p gen/dev
envsubst '$ENV $DOMAIN_NAME $EBS_PERSISTENT_VOLUME_ID $SSL_CERT_ARN $SSL_CERT_ARN_WILDCARD $EC2_TYPE_PROXY $EC2_TYPE_MONOLITH $EC2_TYPE_DATA' < main.tftpl > gen/dev/main.tf
echo "Generated gen/dev/main.tf"

# uat
export ENV="uat"
export DOMAIN_NAME="uat.prism.market"
export EBS_PERSISTENT_VOLUME_ID="vol-043410f6197ee2c31"
...
mkdir -p gen/uat
envsubst '$ENV $DOMAIN_NAME $EBS_PERSISTENT_VOLUME_ID $SSL_CERT_ARN $SSL_CERT_ARN_WILDCARD $EC2_TYPE_PROXY $EC2_TYPE_MONOLITH $EC2_TYPE_DATA' < main.tftpl > gen/uat/main.tf
...
export EBS_PERSISTENT_VOLUME_ID="vol-0e4912ca44f31c1f5"
...
mkdir -p gen/prod
envsubst '$ENV $DOMAIN_NAME $EBS_PERSISTENT_VOLUME_ID $SSL_CERT_ARN $SSL_CERT_ARN_WILDCARD $EC2_TYPE_PROXY $EC2_TYPE_MONOLITH $EC2_TYPE_DATA' < main.tftpl > gen/prod/main.tf
...
echo "cd gen/dev"
echo "export AWS_PROFILE=prism"
echo "export AWS_REGION=us-east-1"

echo "terraform init # if necessary"
echo "terraform apply"
echo "terraform destroy" # if you want to destroy the environment   

echo ""
echo "Note: ensure you have checked in the .sh files!"
echo "git add ./infra"
echo "git commit -m 'Update infra'"
echo "git push"

echo ""
echo "Note: github actions pipeline uploads .sh files to S3 for bootstrapping, so ensure pipeline has run"