...
mkdir -p gen/dev
envsubst '$ENV $DOMAIN_NAME $EBS_VOLUME_ID $SSL_CERT_ARN $SSL_CERT_ARN_WILDCARD $EC2_TYPE_BASTION $EC2_TYPE_PROXY $EC2_TYPE_MONOLITH $EC2_TYPE_DATA' < main.tftpl > gen/dev/main.tf
...
mkdir -p gen/uat
envsubst '$ENV $DOMAIN_NAME $EBS_VOLUME_ID $SSL_CERT_ARN $SSL_CERT_ARN_WILDCARD $EC2_TYPE_BASTION $EC2_TYPE_PROXY $EC2_TYPE_MONOLITH $EC2_TYPE_DATA' < main.tftpl > gen/uat/main.tf
...
mkdir -p gen/prod
envsubst '$ENV $DOMAIN_NAME $EBS_VOLUME_ID $SSL_CERT_ARN $SSL_CERT_ARN_WILDCARD $EC2_TYPE_BASTION $EC2_TYPE_PROXY $EC2_TYPE_MONOLITH $EC2_TYPE_DATA' < main.tftpl > gen/prod/main.tf
