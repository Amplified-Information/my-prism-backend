...
mkdir -p gen/dev
envsubst '$ENV $DOMAIN_NAME $EBS_VOLUME_ID $SSL_CERT_ARN $SSL_CERT_ARN_WILDCARD' < main.tftpl > gen/dev/main.tf
...
export SSL_CERT_ARN="arn:aws:acm:us-east-1:063088900305:certificate/860f7226-32a8-43e3-b03b-2d0f80ac4eb8" # wildcard *.uat.prism.market
...
mkdir -p gen/uat
envsubst '$ENV $DOMAIN_NAME $EBS_VOLUME_ID $SSL_CERT_ARN $SSL_CERT_ARN_WILDCARD' < main.tftpl > gen/uat/main.tf
...
mkdir -p gen/prod
envsubst '$ENV $DOMAIN_NAME $EBS_VOLUME_ID $SSL_CERT_ARN $SSL_CERT_ARN_WILDCARD' < main.tftpl > gen/prod/main.tf
