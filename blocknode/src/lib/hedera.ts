...
const pubAllEventsForContract = async (stack: Dedupe, nats: NatsConnection, network: LedgerId, contractId: ContractId, fromTimestampUnixtimeSeconds: number, toTimestampUnixtimeSeconds: number/* = (Math.ceil(new Date().getTime() / 1000) + 1) default: current time + 1 second */) => {
  ...
  const url = `https://${net}.mirrornode.hedera.com/api/v1/contracts/${cid}/results/logs?timestamp=gt:${fromTimestampUnixtimeSeconds}&timestamp=lt:${toTimestampUnixtimeSeconds}.000000000`
  // console.log(url)
  try {
    ...
    log.debug('Fetched events for contract', { network: net, contractId: cid, eventCount: events.length })
